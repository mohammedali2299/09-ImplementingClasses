...
class Point(object):
    ...
    def get_distance_from(self, p1):
        p1 = Point(p1.x, p1.y)
        d = math.sqrt((p1.x - self.x)**2 + (p1.y - self.y)**2)
        return d

    def get_distance_from_start(self):
        d = math.sqrt((self.x - self.dx)**2 + (self.y - self.dy)**2)
        return d
    ...
    def closer_to(self, p2, p3):
        two = math.sqrt((self.x - p2.x)**2 + (self.y - p2.y)**2)
        three = math.sqrt((self.x - p3.x)**2 + (self.y - p3.y)**2)
        if two >= three:
            return p2
        else:
            return p3
